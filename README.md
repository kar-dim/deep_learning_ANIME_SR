...
Σημαντικό: αυτή η υστέρηση εμφανίζεται **μόνο στο PSNR**. Στο perceptual **LPIPS** και τα δύο EDSR μοντέλα είναι καλύτερα από το bicubic σε **όλες τις 188 εικόνες**, συμπεριλαμβανομένων και αυτών των 13 που "χάνουν" στο PSNR. Στις flat/θολές αυτές εικόνες το EDSR προσθέτει ελάχιστη υφή που αποκλίνει ελαφρά από τις ακριβείς τιμές pixel του ground truth (το PSNR επειδή είναι pixel-level το τιμωρεί), αλλά το αποτέλεσμα δεν "φαίνεται" χειρότερο, οπότε ένα δίκτυο αντίληψης (LPIPS) δεν το τιμωρεί. Πρακτικά, η "υστέρηση" είναι αποτέλεσμα της pixel-level PSNR μετρικής και όχι πραγματική οπτική υποβάθμιση. Για αυτόν τον λόγο είναι πολύ χρήσιμα τα perceptual metrics για Super Resolution.
